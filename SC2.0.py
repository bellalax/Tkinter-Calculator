...
for j in range(1,4):
    for k in range(3):
        ...
        bttn[i].grid(row = j, column = k, sticky=EW, columnspan=2)
...
bttn_div.grid(row = 6, column = 6)

bttn_mult = Button(calc, text = "x")
bttn_mult["command"] = lambda: sum1.operation("times")
bttn_mult.grid(row = 5, column = 6, sticky=EW, columnspan=2)

minus = Button(calc, text = "-")
minus["command"] = lambda: sum1.operation("minus")
minus.grid(row = 4, column = 6, sticky=EW, columnspan=2)

point = Button(calc, text = ".")
point["command"] = lambda: sum1.num_press(".")
point.grid(row = 6, column = 2, sticky=EW, columnspan=2)

add = Button(calc, text = "+")
add["command"] = lambda: sum1.operation("add")
add.grid(row = 1, column = 4, sticky=EW, columnspan=2)
...
equals.grid(row = 6, column = 3, sticky=EW, columnspan=2)
